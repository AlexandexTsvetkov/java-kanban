...
public class HttpTaskServerPrioritisedTest {
    ...
    @Test
    public void testGetPrioritised() throws IOException, InterruptedException {
        // создаём задачу
        ...
        URI url4 = URI.create("http://localhost:8080/api/v1/prioritized");
        ...
        assertEquals(gson.toJson(manager.getPrioritizedTasks()), response4.body());
    }

    @Test
    public void testGetPrioritisedBadRequest() throws IOException, InterruptedException {
        // создаём задачу
        Task task = new Task("Test 2", "Testing task 2", LocalDateTime.now(), Duration.ofMinutes(5));
        // конвертируем её в JSON
        String task1Json = gson.toJson(task);
        ...
        URI url4 = URI.create("http://localhost:8080/api/v1/prioritized/1");
        HttpRequest request4 = HttpRequest.newBuilder().uri(url4).GET().build();
